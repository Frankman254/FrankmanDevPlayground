...
export function ChessGame() {
	...
	const [boardViewportSize, setBoardViewportSize] = useState<number | null>(
		null
	);
	...
	useEffect(() => {
		...
		const measure = () => {
			...
			const fileLabelReserve = 24;
			const nextSize = Math.max(
				Math.floor(
					Math.min(rect.width, rect.height - fileLabelReserve)
				),
				0
			);
	...
	const whiteCaptures = useMemo(
		() => getCapturedPieces(game, 'white'),
		[game]
	);
	const blackCaptures = useMemo(
		() => getCapturedPieces(game, 'black'),
		[game]
	);
	...
	const ranks =
		orientation === 'white' ? BOARD_RANKS : [...BOARD_RANKS].reverse();
	const files =
		orientation === 'white' ? BOARD_FILES : [...BOARD_FILES].reverse();
	const canInteract = game.status === 'active' || game.status === 'check';
	const boardInteractive = canInteract && !pendingPromotion && !cpuThinking;
	const lastMove = game.lastMove;
	const selectionPiece = selectedSquare
		? getPieceAt(game, selectedSquare)
		: null;
	const fullscreenBoardSize =
		isFullscreen && boardViewportSize
			? {
	...
	function handleSquareClick(square: Square) {
		...
		if (selectedSquare) {
			const matchingMoves = selectedMoves.filter(
				move => move.to === square
			);
	...
	return (
		<div
			className={cn(
				fs &&
					'flex max-h-[100dvh] min-h-0 w-full flex-col overflow-hidden bg-[#0C0C0F] p-2 sm:p-4'
			)}
			ref={shellRef}
		>
			<div
				...
				>
					<SurfaceCard
						className={cn(
							...
							fs && 'shrink-0 space-y-4 p-4 sm:p-5'
						)}
					>
						<motion.div
							...
							<div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
								...
								{cpuEnabled ? (
									<div className="flex flex-wrap items-center gap-2">
										<span className="text-xs text-[#F7F3EB]/55">
											{t.chess.humanColorLabel}
										</span>
										<Button
											onClick={() =>
												setHumanColor('white')
						...
						>
							...
							<div className="flex flex-col items-end gap-1 sm:items-end">
								...
								{cpuEnabled && cpuThinking ? (
									<p className="text-xs text-[#F5C400]/90">
										{t.chess.cpuThinking}
									</p>
								) : null}
							</div>
						...
						>
							...
							>
								...
								>
									<div
										className={cn(
											'grid aspect-square w-full grid-cols-8 gap-0.5 sm:gap-1',
											fs && 'mx-auto max-w-full shrink-0'
										)}
										style={fullscreenBoardSize}
									>
										{ranks.flatMap(rank =>
											files.map(file => {
												const square =
												...
												return (
													<motion.button
														className={cn(
															...
															isSelected &&
																'border-[#F5C400] ring-2 ring-[#F5C400]/55',
															isTarget &&
																'border-emerald-300/45',
															isLastMove &&
																'border-sky-300/35',
															isCheckedKing &&
																'border-rose-400/65 ring-2 ring-rose-400/45',
															!boardInteractive &&
					...
					<SurfaceCard className="space-y-5">
						...
						<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-1">
							...
							<StatPill
								...
								value={
									selectionPiece
										? `${selectionPiece.color} ${selectionPiece.type}`
										: t.chess.noSelection
								}
							/>
							<StatPill
					...
					<SurfaceCard className="space-y-5">
						...
						<div className="space-y-2">
							{game.history.length === 0 ? (
								<p className="text-sm text-[#F7F3EB]/60">
									{t.chess.emptyHistory}
								</p>
							) : (
	...
	switch (game.status) {
		...
		case 'checkmate':
			return game.winner === 'white'
				? t.chess.whiteWinsByMate
				: t.chess.blackWinsByMate;
		case 'stalemate':
